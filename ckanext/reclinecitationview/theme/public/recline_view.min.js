this.ckan.module('recline_view',function(jQuery){return{options:{site_url:"",controlsClassName:"controls",dataproxyUrl:"//jsonpdataproxy.appspot.com"},onCiteSubset:function(event){ds=event.data.dataset;resource_id=event.data.resource_id;attributes=event.data.dataset.queryState.attributes
prepared_filters={}
attributes.filters.forEach(function(el){prepared_filters[el.field]=el.term});event.data.client.call('POST','issue_pid',{'resource_id':resource_id,'statement':prepared_filters,'sort':attributes.sort},function(data){url='/querystore/view_query?id='+data.result;$('#cite-response-text').html("A background job was triggerd to calculate the hash value of the resultset. Once the job was successful all meta information of the subset can be found <a target=\"_blank\" href='"+url+"'>here</a>");$('#cite-response-panel').fadeIn();});},query:null,initialize:function(){jQuery.proxyAll(this,/_on/);this.options.resource=JSON.parse(this.options.resource);this.options.resourceView=JSON.parse(this.options.resourceView);this.el.ready(this._onReady);L.Icon.Default.imagePath=this.options.site_url+'vendor/leaflet/0.7.7/images';},_onReady:function(){var resourceData=this.options.resource,resourceView=this.options.resourceView;this.loadView(resourceData,resourceView);},loadView:function(resourceData,reclineView){var self=this;function showError(msg){msg=msg||self._('error loading view');window.parent.ckan.pubsub.publish('data-viewer-error',msg);}
...
dataset=new recline.Model.Dataset(resourceData);map_config=this.options.map_config;query=new recline.Model.Query();query.set({size:reclineView.limit||100});query.set({from:reclineView.offset||0});var urlFilters={};try{if(window.parent.ckan.views&&window.parent.ckan.views.filters){urlFilters=window.parent.ckan.views.filters.get();}}catch(e){}
...
dataset.fetch().done(function(dataset){self.initializeView(dataset,reclineView);}).fail(function(error){if(error.message)errorMsg+=' ('+error.message+')';showError(errorMsg);});},initializeView:function(dataset,reclineView){var view,state,controls=[];this.$('#cite-btn').click({"dataset":dataset,"resource_id":this.options.resource.id,"client":this.sandbox.client},this.onCiteSubset)
...
view=new recline.View.Map($.extend(this._reclineMapViewOptions(dataset,this.options.map_config),{state:state}));}else if(reclineView.view_type==="reclinecitation_view"){view=this._newDataExplorer(dataset,this.options.map_config);}else{view=new recline.View.SlickGrid({model:dataset});controls=[new recline.View.Pager({model:view.model}),new recline.View.RecordCount({model:dataset}),new recline.View.QueryEditor({model:view.model.queryState})];}
if(reclineView.view_type!=='reclinecitation_view'){var newElements=jQuery('<div />');this._renderControls(newElements,controls,this.options.controlsClassName);newElements.append(view.el);jQuery(this.el).html(newElements);view.visible=true;view.render();}
